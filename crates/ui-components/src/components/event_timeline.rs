    ...
    rsx! {
        section { class: "flex flex-col gap-4",
            div { class: "flex items-end justify-between gap-3 px-1",
                ...
                p { class: "text-sm text-base-content/50", "按时间顺序展示运行、输出、错误和恢复信息" }
            }
                    ...
                    div { class: "grid gap-3 p-4 md:grid-cols-[auto_minmax(0,1fr)]",
                        ...
                        div { class: "min-w-0",
                            div {
                                ...
                                span { class: "badge badge-ghost border-none bg-base-200/80 px-3 py-3 text-xs font-medium text-base-content/62", "{entry.meta}" }
                            }
                            p { class: "mt-3 whitespace-pre-wrap text-[15px] leading-7 text-base-content/76", "{entry.body}" }
                            div { class: "mt-4 h-px w-full bg-base-300/80", "" }
                        }
                    }
                }
            }
...
fn timeline_marker_class(tone_class: &str) -> &'static str {
    if tone_class.contains("error") {
        ...
        "mt-1 h-2.5 w-2.5 rounded-full bg-base-300"
    }
}
